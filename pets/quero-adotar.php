        ...
        <div class="container-fluid">
            <div class="row">
                <aside style="width: 320px;">
                    <form method="" class="bg-custom rounded p-3 text-uppercase pt-4 mt-2 position-sticky"
                        style="top: 1rem;">
                                ...
                                <div class="p-3">
                                    ...
                                    <div class="d-flex align-items-center gap-2 mt-2 py-2">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                            ...
                                            <path fill="#006AB0" fill-rule="evenodd"
                                ...
                                <div class="p-3">
                                    ...
                                    <div class="d-flex align-items-center gap-2 mt-2 py-2">
                                        ...
                                        <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18"
                                            ...
                                            <path fill="#FF7373" fill-rule="evenodd"
...
<?php
    ...
    {
        global $conn;
        $sql = "SELECT * FROM animais WHERE status = 'Ativo'";
    ...
    // Função para listar solicitações de adoção
    function listarAdocoes()
    {
        global $conn;
        $sql = "SELECT * FROM adocoes";
        $result = $conn->query($sql);
        $adocoes = array();
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
    ...
    // Função para cadastrar solicitação de adoção
    function cadastrarAdocao($nome, $id_animal, $cpf, $email, $celular, $data_nascimento)
    {
        global $conn;
        $stmt = $conn->prepare("INSERT INTO adocoes (nome_solicitante, id_animal, cpf, email, celular, data_nascimento) VALUES (?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("siisss", $nome, $id_animal, $cpf, $email, $celular, $data_nascimento);
        $stmt->execute();
        $stmt->close();
    }

    // Função para cadastrar animal
    function cadastrarAnimal($nome, $especie, $raca, $idade, $peso, $porte, $local, $sobre, $status)
    {
        global $conn;
        $stmt = $conn->prepare("INSERT INTO animais (nome, especie, raca, idade, peso, porte, local, sobre, status) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("sssiissss", $nome, $especie, $raca, $idade, $peso, $porte, $local, $sobre, $status);
        $stmt->execute();
        $stmt->close();
    }

    // Função para verificar se o animal já foi cadastrado
    function verificarAnimalExistente($nome, $especie, $idade)
    {
        global $conn;
        $stmt = $conn->prepare("SELECT * FROM animais WHERE nome = ? AND especie = ? AND idade = ?");
        $stmt->bind_param("ssi", $nome, $especie, $idade);
        $stmt->execute();
