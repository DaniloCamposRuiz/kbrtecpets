        ...
        <main class="col h-100 text-light p-4">
            ...
            <form action="" method="post" class="bg-custom rounded col-12 py-3 px-4">
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="text" name="usuario" class="form-control bg-dark text-light border-dark"
                            id="usuario" placeholder="Ex: Admin">
                    </div>
                </div>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="email" name="email" class="form-control bg-dark text-light border-dark" id="email"
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="password" name="senha" class="form-control bg-dark text-light border-dark"
                            id="senha">
                    </div>
                </div>
                ...
                <div class="mb-3 row">
                    ...
                    <div class="col-sm-10">
                        <input type="password" name="confSenha" class=" form-control bg-dark text-light border-dark"
                            id="confSenha">
                    </div>
                </div>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($usuario) || empty($senha)) {
        ...
        $checkUserStmt = $conn->prepare("SELECT id FROM usuarios WHERE usuario = ?");
        $checkUserStmt->bind_param("s", $usuario);
        $checkUserStmt->execute();
        $checkUserStmt->store_result();

        // Verificar se já existe um e-mail igual na base de dados
        ...
        if ($checkUserStmt->num_rows > 0) {
            ...
            $insertStmt = $conn->prepare("INSERT INTO usuarios (usuario, email, password) VALUES (?, ?, ?)");
            $insertStmt->bind_param("sss", $usuario, $email, $hashed_password);

            if ($insertStmt->execute()) {
                echo "Usuário cadastrado com sucesso!";
            } else {
                echo "Erro ao cadastrar o usuário: " . $insertStmt->error;
            }
        ...
        // Fechar as declarações de verificação e a conexão
        $checkUserStmt->close();
